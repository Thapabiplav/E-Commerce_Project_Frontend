...
const[date,setDate]=useState<string>('')
  const filterOrders =myOrders.filter((order)=> selectedItem === OrderStatus.All || order.orderStatus === selectedItem).filter((order)=>order.id.toLowerCase().includes(searchItem) ||order.Payment.paymentMethod.toLowerCase().includes(searchItem) || order.totalAmount.toString().includes(searchItem)).filter((order)=> date === '' || new Date(order.createdAt).toLocaleDateString() === new Date (date).toLocaleDateString()) 
  return (
    <>
      <Navbar />
            ...
            <div className="my-2 flex sm:flex-row flex-col">
              <div className="flex flex-row mb-1 sm:mb-0">
                <div className="relative">
                  <select onChange={(e)=>setSelectedItem(e.target.value as OrderStatus)}className="appearance-none h-full rounded-r border-t sm:rounded-r-none sm:border-r-0 border-r border-b block  w-full bg-white border-gray-400 text-gray-700 py-2 px-4 pr-8 leading-tight focus:outline-none focus:border-l focus:border-r focus:bg-white focus:border-gray-500">
